...
description: "Operational controls, runtime configuration, governance safety model, and production checklist for running s2s in chat-first mode."
---

## Operating mode

The current operational mode is chat-first governance with project-local runtime in `.s2s`.
...
3. Keep the command allowlist minimal.
4. Keep `allowUnsafeRawCommand` disabled.
5. Set bounded execution timeouts.
6. Keep human approval for merge decisions.
7. Enforce branch safety before push: if the current branch is tied to closed/merged PRs, switch to a fresh branch and open a new PR.
...
- Verify the selected chat CLI is installed and authenticated.
