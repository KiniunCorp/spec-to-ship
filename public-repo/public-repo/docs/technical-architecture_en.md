# Spec-To-Ship Technical Architecture (EN)

## Architecture overview

Spec-To-Ship uses a three-layer orchestration model:
...
Setting `pipelineMode: 'standalone'` in `runtime.json` causes `s2s stage <stage>` to call the LLM API directly (API mode only). This is an explicit opt-in for headless CI environments. See [LLM Access Modes](./llm-access-modes_en.md) for configuration details.
...
The orchestrator determines which stages are needed for a given request — not all requests require all stages.
