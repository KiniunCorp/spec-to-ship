...
description: "s2s usa un modelo de orquestación en tres capas: CLI shell, Flow Orchestrator y un plano de ejecución para trabajo de engineering aislado en git."
---

## Vista general

s2s usa un modelo de orquestación en tres capas:
...
Configurar `pipelineMode: 'standalone'` en `runtime.json` hace que `s2s stage <etapa>` llame directamente a la API del LLM. Es una opción explícita para entornos CI sin sesión de chat interactiva. Ver [Modos de Acceso LLM](/es/llm-access-modes/) para detalles de configuración.
...
El orquestador determina qué etapas necesita una solicitud dada — no todas las solicitudes requieren todas las etapas.

## Flujo del orquestador

`s2s request "<prompt>"` o `s2s stage <etapa>` dispara:
