# Spec-To-Ship — Cómo Funciona

## A. Descripción corta del producto
...
## D. Cómo funciona el flujo de trabajo
...
Desde tu sesión de chat AI — o directamente desde la terminal — envías una solicitud: `s2s request "agregar rate limiting a la API"`. El orquestador lee la solicitud, clasifica el intent (¿nueva funcionalidad? ¿bug fix? ¿investigación?) y decide el conjunto mínimo de etapas que ese trabajo realmente necesita. Te dice: "la ruta es `pm → engineering → engineering_exec`, aprobación requerida antes de ejecución."
...
## E. Por qué este flujo de trabajo es valioso
