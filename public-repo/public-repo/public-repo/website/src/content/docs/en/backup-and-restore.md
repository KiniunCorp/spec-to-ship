...
description: "Back up and restore the project governance and runtime state managed by s2s, with automatic safety snapshots before every restore."
---
...
Tip: `s2s backup` output prints the snapshot id and project backup directory.
...
4. Note that `s2s` also creates automatic pre-policy snapshots when guardrail discrepancy decisions are applied in init/config.
